...
function strengthScale(emotion, value = 3) {
  return `<div class="strength-item" data-for="${emotion}"><div class="strength-head"><span>${emotion}</span><b>${value}</b></div><div class="scale-wrap"><input class="strength-input" data-emotion="${emotion}" type="range" min="1" max="5" step="1" value="${value}" style="--strength:${(value - 1) / 4 * 100}%"><div class="scale-labels"><span>1</span><span>2</span><span>3</span><span>4</span><span>5</span></div><div class="scale-ends"><small>轻微</small><small>强烈</small></div></div></div>`;
}
function assessment() {
  ...
  document.querySelectorAll('.strength-input').forEach(input => input.oninput = () => { const i = q.emotions.indexOf(input.dataset.emotion); q.strengths[i] = +input.value; input.style.setProperty('--strength', `${(+input.value - 1) / 4 * 100}%`); input.closest('.strength-item').querySelector('.strength-head b').textContent = input.value; changes++; });
